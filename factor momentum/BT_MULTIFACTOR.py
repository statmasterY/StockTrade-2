...
class Task(backtest.BaseBacktestTask):
    def execute(self):
        try:
            ...
            # factor_quantile= 5 # 股票分组数 备选：5,10
            # method_weight = 'equal' # 股票权重分配方法  备选：'return','equal'
            # method_factor = 'long_short' # 因子收益率计算方法 备选：'long','long_only','short_only','long_short'
            # backdays = 252 # 因子收益率计算周期 备选：252,126,31
            # period = 5 # 调仓周期 备选：5,10,20
            # if_industry_market_cap_norm = True #是否需要市值行业中性化，备选：False（新增）
            ...
            ####解包参数，写为args形式
            self.taskID, self.FactorData, self.args \
                = list(itemgetter('TASKID', 'FactorData', 'Arguments')(self.params))
            nums_xg, nums_fin, factor_quantile, method_weight, method_factor, backdays, period, if_industry_market_cap_norm, if_xgboost, n_splits, verbose, index, y_index \
                = tuple(self.args.values())
            ...
            self.data_dir = self.gen_data_dir() #后面待修改，不知道存储文件的路径应该怎么设置
            ...
            # 所有因子的特征名称,去掉return,return_norm列
            all_features = factordata_df.columns.drop(['return','return_norm']) #做了修改
            self.progress(30)

            
            ####计算因子收益率矩阵（做了修改调整）
            def factor_return(df, factor_name=all_features, factor_quantile=5, method = 'long_short', backdays = 252):
            ...
            def feature_collect(fac_mom, factors_r_all, factor_name=all_features, nums_fin = 10):
                coefficients = []
                for factor in factor_name:
                    X = sm.add_constant(fac_mom[factor])
            ...
            if if_xgboost:
                # 用xgboost来筛选特征
                top_features, coefficients = feature_collect(fac_mom, factors_r_all, factor_name=top_20_features, nums_fin=nums_fin)
                ##根据筛选出的因子，重新计算因子收益率矩阵
                ...
                # 用回归系数来筛选特征
                top_features, coefficients = feature_collect(fac_mom, factors_r_all, factor_name=all_features, nums_fin=nums_fin)
                ##根据筛选出的因子，重新计算因子收益率矩阵
                factors_r, factordata_df, fac_mom, fac_mom_ternary = factor_return(factordata_df,factor_name=top_features, factor_quantile=factor_quantile,method=method_factor, backdays=backdays)
            ...
            def plot_cumprod(cumprod_r, name):
                ...
                dir = os.path.join(self.data_dir, f'factor_returns_{name}.png') # 保存图片的路径，后面debug
                plt.savefig(dir)
                plt.close()
                self.log.record(f'成功存储图片到路径：{dir}')
            
            plot_cumprod(factors_r_all, name = 'all_factors')
            #plot_cumprod(factors_r_all[[*top_20_features]], name = 'XGBoost')
            plot_cumprod(factors_r_all[[*top_features]], name = 'Regression')
            ...
            def get_weight(fac_mom,fac_mom_ternary,method_weight = 'equal',method_factor = 'long'):  ###做了修改调整
                ...
                if method_weight == 'equal':
                    ...
                    weight = (fac_mom-1).shift(1).apply(lambda x : trans(x,method_factor=method_factor), axis=1)
            ...
            # 都是当日收盘时的权重
            factor_weight = get_weight(fac_mom,fac_mom_ternary,method_weight = method_weight,method_factor = method_factor)
            stock_weight = get_stock_weight(factordata_df, method_weight=method_weight) 
    
            self.progress(70)

            # return of F1 for group 1 in day 1 = sum(sum(weight * return of stock in group 1(group by f1) in day 1) * weight of factor i in day 1)
            # return of group 1 in day 1 = weight of stocks'  np.dot  [return * (dfgroup== 1)]  * weight of factor i in day 1
            #                              1 * 574                    (574*1   574*20 )= 574*20       20*1
            # return of group 1 in day 1 = np.dot(np.dot(weight_stock[day1],df4[day1]['return']*df4[day1][group columns==1]), weight_factor[day1])
           
            ###############################  合并成组合中每日更新的股票的权重
            ###############################  合并成每个组合中股票的权重
            ###############################  合并成每个组合中股票的权重
            def get_weight_stock_final(factpr_df,stock_weight,factor_weight,method_factor='long',quantile=5):
                #数据预处理
                df = factpr_df.copy()
                ...
                # # long only
                # factor_weight[factor_weight<0] = factor_weight[factor_weight<0]*(method != 'long_only')
                # # short only
                # factor_weight[factor_weight>0] = factor_weight[factor_weight>0]*(method != 'short_only')
                # # factor_weight归一化
                # factor_weight = factor_weight.apply(lambda x: x/sum(abs(x)), axis=1)
                ...
                if method_factor == 'long_short': ##构建零投资组合
                    for date in stock_weight.index:
                        ...
                        # 保留所有weight，按因子加权求和
                        # 原版：dsf = d.mul(np.sign(f),axis=1).fillna(0).apply(lambda x: x/x.abs().sum(), axis=0)
                        # 按因子加权求和
                        dsf = d.mul(np.sign(f),axis=1).fillna(0).apply(lambda x : trans(x,method_factor='long_short'), axis=0)
            ...
            stock_weight_tol = get_weight_stock_final(factordata_df,stock_weight,factor_weight,method_factor=method_factor,quantile=factor_quantile) #没有去除nan
            stock_weight_tol = stock_weight_tol.dropna(how='all')
            stock_weight_tol = stock_weight_tol.fillna(0)  ####后面这里写成unstack的形式和机器小组统一起来
            

            stock_weight.to_csv(os.path.join(self.data_dir, 'stock_weight.csv'))
            factor_weight.to_csv(os.path.join(self.data_dir, 'factor_weight.csv'))  
            stock_weight_tol.to_csv(os.path.join(self.data_dir, 'stock_weight_tol.csv'))  
            ...
            portfolio_r.to_csv(os.path.join(self.data_dir, 'portfolio_return.csv'))
            ...
            qs.reports.html(bt_data.total_return, output=os.path.join(self.data_dir, 'Res_'+method_weight+'_'+method_factor+now+'.html'), title='Backtest Result')

            # 超额收益
            qs.reports.html(bt_data.total_return - bt_data.benchmark, output=os.path.join(self.data_dir, 'extra return'+method_weight+'_'+method_factor+str(now)+'.html'), title='Extra return Result')

            # 把total_return和benchmark用qs.reports.html画图
            qs.reports.html(bt_data[['total_return', 'benchmark']], output=os.path.join(self.data_dir, 'Res_2_'+method_weight+'_'+method_factor+now+'.html'), title='Backtest Result')

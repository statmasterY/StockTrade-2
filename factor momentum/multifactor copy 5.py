...
backdays = 126 # 因子收益率计算周期 备选：252,126,31
...
sample_weight = 'equal_weight' # 样本权重 备选：'integer_weight','equal_weight'
if_indicators = False # 是否使用正负1来回归 备选：True,False
...
#%%
# 使用长数据
df = pd.read_csv("F:\data\ALL_factorValues_2019-2021.csv")
df2 = pd.read_csv("F:\data\ALL_factorValues_2021-2023.csv") 
# df2 = pd.read_csv('F:\data\All_factorValues_2021_2023_filter_7.csv',encoding='gbk')
df3 = pd.read_pickle("F:\data\All_factorValues_OriginalIndex_updated_0705.pkl")
pools = pd.read_csv("F:\data\pool.csv") # 股票池里有权重的股票当作主动风格的，用来打标签
aggresive = pools.loc[pools.weight>0,'qvcode'].unique() # 主动风格股票代码

df['date']=pd.to_datetime(df['date']).dt.date
df.set_index(['date', 'qscode'], inplace=True)
df2['date']=pd.to_datetime(df2['date']).dt.date
df2.set_index(['date', 'qscode'], inplace=True)
df3['date']=pd.to_datetime(df3['date']).dt.date
df3.set_index(['date', 'qscode'], inplace=True)

# 合并两个数据集
factordata_df = pd.concat([df, df2, df3], axis=0)
del df, df2, df3

# 删除重复索引的行
factordata_df = factordata_df[~factordata_df.index.duplicated(keep='first')]

# # 使用短数据
# factordata_df = pd.read_csv("F:\data\ALL_factorValues_2021-2023.csv") 
...
####计算因子收益率矩阵（做了修改调整）
def factor_return(df, factor_name=top_20_features, factor_quantile=5, method_return_cal = 'long_short', backdays = 252, return_period = 5):
    ...
    fac_mom_ternary = fac_mom_l.copy() #先不用dropna
...
#         backdays=backdays,return_period = 5)
################################################

#%%
# factordata_df.to_pickle(r'F:\data\factordata_df.pkl')
factordata_df = pd.read_pickle(r'F:\data\factordata_df.pkl')
all_features = factordata_df.columns.drop(['return','return_norm','forward_return_raw','forward_return_norm'])[:88]
top_20_features = ['exp_wgt_momentum_1m', 'trix_15', 'bb_pos_20', 'williams_20', 'macd_histogram_12_26', 'macd_12_26', 'cci_20', 'wgt_momentum_1m', 'eom_20', 'vol_std_21', 'turnover_std_1m', 'vr_20', 'ma_20', 'bb_range_20', 'exp_wgt_momentum_3m', 'vol_std_63', 'cmo_20', 'qstick_20', 'pet_20', 'ep']
# factors_r_all.to_csv(r'F:\data\factors_r_all.csv')
factors_r_all = pd.read_csv(r'F:\data\factors_r_all.csv')
factors_r_all['date']= pd.to_datetime(factors_r_all['date']).dt.date
factors_r_all = factors_r_all.set_index('date')
fac_mom_l = (factors_r_all + 1).rolling(backdays).apply(lambda x: x.prod()) # day1用到了day2的return
fac_mom_s = (factors_r_all + 1).rolling(return_period).apply(lambda x: x.prod())
fac_mom_ternary = fac_mom_l.copy() #先不用dropna
fac_mom_ternary[fac_mom_ternary <= 1] = -1
fac_mom_ternary[fac_mom_ternary > 1] = 1
...
# def feature_collect(fac_mom_l, fac_mom_s,factor_name = top_20_features, nums_fin = 10, start_date = datetime.date(2022,6,15), return_period = 7,sample_weight = 'integer_weight',if_indicators=False):
#     coefficients = []
#     t = []
#     # df 是 fac_mom, 取start_date之前的数据 ,shift 1 day,避免使用未来数据
#     df = fac_mom_l.shift(1+return_period).loc[:start_date] # 过去一年的收益率不应包含当前考察时间段内的收益率，所以shift 1+period day
...
#%%
# 计算每个因子的权重
def get_weight(fac_r,fac_mom_l,fac_mom_s,fac_mom_63,fac_mom_ternary,coef_df,top_features,method_weight = 'equal',method_factor = 'long'):  ###做了修改调整
    """
    ...
    if method_weight == 'equal':
        ...
        # 按预期的未来5日收益率来分配当日的权重
        # weight = ((fac_mom_l*coef_df['coefficients']+coef_df['const'])[top_features]-1)\
        #    .shift(1).apply(lambda x : trans(x,method_factor=method_factor), axis=1)
...
def get_weight_stock_final(factor_df,stock_weight,factor_weight,method_factor='long',quantile=5):
    ...
    is_limit = df['return'].unstack(level='qscode').apply(lambda x: ((x.abs() > 0.0993) & (x.abs() < 0.1004)) | (x.abs() > 0.1991), axis=1) \
        * np.sign(df['return'].unstack(level='qscode'))
...
def feature_collect(factors_r_all,fac_mom_l, fac_mom_s,factor_name = top_20_features, nums_fin = 10, start_date = datetime.date(2022,6,15), return_period = 7,sample_weight = 'integer_weight',if_indicators=False):
    ...
    for factor in factor_name:##后续改为factor_name
        ...
        # plt.scatter(X2[factor], y2, color='blue', label='Actual')
        # plt.plot(X2[factor], predicted_values, color='red', label='Predicted')
        # plt.plot(X2[factor], predicted_values + 1*np.sqrt(sigma), color='blue', label='up')
        # plt.plot(X2[factor], predicted_values - 1*np.sqrt(sigma), color='blue', label='down')
        # plt.plot(X2[factor], predicted_values + 1.5*np.sqrt(sigma), color='yellow', label='up')
        # plt.plot(X2[factor], predicted_values - 1.5*np.sqrt(sigma), color='yellow', label='down')
        # plt.xlabel('Predicted Values '+factor)
        # plt.ylabel('Actual Values '+str(y2.index[0]))
        # plt.legend()
        # plt.show()
...
plot_cumprod(factors_r_all.loc[(datelist[start_index]<factors_r_all.index.get_level_values(0)) & (factors_r_all.index.get_level_values(0)<=datelist[end_index])][[*top_features]])
fac_mom_l22 = fac_mom_l.loc[(datelist[start_index]<fac_mom_l.index.get_level_values(0)) & (fac_mom_l.index.get_level_values(0)<=datelist[end_index])][[*top_features]]
# fac_mom_l22.plot(figsize=(20,10),title='factor_quantile_1')
fac_mom_s22 = fac_mom_s.loc[(datelist[start_index]<fac_mom_s.index.get_level_values(0)) & (fac_mom_s.index.get_level_values(0)<=datelist[end_index])][[*top_features]].iloc[-21:]
# fac_mom_s22.plot(figsize=(20,10),title='factor_quantile_2')
...
for i in range(end_index, len(datelist), factor_period):
    ...
    # 因子适用的日期
    # print('factor period: ',datelist[end_index-factor_period+1],datelist[end_index])

    # 需要的分组数据
    factor_df2 = factor_df[[*top_features_qt ,'return_norm','return','forward_return_norm','forward_return_raw']]

    # 都是当日收盘时的权重
    factor_weight = get_weight(fac_r,fac_mom_l2[[*top_features]],fac_mom_s2[[*top_features]],fac_mom_632[[*top_features]],fac_mom_ternary2[[*top_features]],coef_df,top_features,method_weight = method_weight,method_factor = method_factor).loc\
        [datelist[end_index-factor_period+1]:datelist[end_index]]
    stock_weight = get_stock_weight(factor_df2, method_weight=method_weight).loc\
        [datelist[end_index-factor_period+1]:datelist[end_index]]
...
# 保存数据
# weight_factor.to_csv('F:\data\weight_factor.csv')
# weight_stock_final.to_csv('F:\data\weight_stock_final.csv')

# 绘制因子权重
weight_factor.plot(figsize=(20,10))
plt.show()
...
# 回测的开始和结束时间
start_date = weight_stock_final.dropna().index[0]
...
para = 'period'+str(period)+'_groupnums:'+str(factor_quantile)+'_method:'+method_factor+'_'+method_weight+'_backdays:'+str(backdays)

# 把total_return和benchmark用qs.reports.html画图
qs.reports.html(bt_data[['total_return','benchmark']], output=path+'Res_2_'+now+'.html', title='Backtest Result_'+para)

# stock_weight_tol.to_csv('F:\data\stock_weight_tol'+now+'.csv',encoding='gbk')
